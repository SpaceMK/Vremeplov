...
{
    /// <summary>
    /// C# port of the mapbox/earcut polygon triangulation library (ISC licence).
    /// Triangulates a 2D polygon with optional holes given as a flat coordinate
    /// array. Returns triangle indices into the vertex list (vertex i = data[2i],
    /// data[2i+1]). https://github.com/mapbox/earcut
    /// </summary>
    public static class Earcut
    {
        ...
        static bool IsEar(Node ear)
        {
            ...
            double x0 = Min3(ax, bx, cx), y0 = Min3(ay, by, cy), x1 = Max3(ax, bx, cx), y1 = Max3(ay, by, cy);
        ...
        static bool IsEarHashed(Node ear, double minX, double minY, double invSize)
        {
            ...
            double ax = a.x, bx = b.x, cx = c.x, ay = a.y, by = b.y, cy = c.y;
            double x0 = Min3(ax, bx, cx), y0 = Min3(ay, by, cy), x1 = Max3(ax, bx, cx), y1 = Max3(ay, by, cy);
            double minZ = ZOrder(x0, y0, minX, minY, invSize), maxZ = ZOrder(x1, y1, minX, minY, invSize);
        ...
        static Node CureLocalIntersections(Node start, List<int> triangles, int dim)
        {
            Node p = start;
            do
            {
        ...
        static double ZOrder(double xd, double yd, double minX, double minY, double invSize)
        {
        ...
        static double Min3(double a, double b, double c) => System.Math.Min(a, System.Math.Min(b, c));
        static double Max3(double a, double b, double c) => System.Math.Max(a, System.Math.Max(b, c));
    }
}
